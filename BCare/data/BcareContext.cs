...
namespace BCare.data
{
    public class BcareContext
    {
        ...
        public void SignUp(int User_ID, string First_Name, string Last_Name, string Gender, string Birth_Date, int HMO_ID, string Blood_Type, string Address)
        {
            ...
            {
                conn.Open();
                MySqlCommand cmd = new MySqlCommand("INSERT INTO users VALUES (User_ID, First_Name, Last_Name, Gender, Birth_Date, 'FOREIGN KEY (HMO_ID) REFERENCES health_maintenance_organizations (HMO_ID)', Blood_Type, Address) ", conn);
                cmd.ExecuteNonQuery();
            }
           // return newUser;
        }
